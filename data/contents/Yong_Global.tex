...
本小节将讨论方程\eqref{eq:compressibleRelax}解的存在性。这里我们用$W=(\rho,\rho v,\rho F,\rho c)$表示上面的$U$。而利用$U$表示$U = (\rho, v, F, c)。$首先由$s$的形式为
\begin{equation*}
...
\begin{equation*}
    s_0(\nu) = \int_{\rho_0}^{1/\nu} \frac{p(z)}{z^2} dz.
\end{equation*}
由有限形变的守恒-耗散理论我们无法得到$s_{UU}$可以将$U$的方程对陈化。然而由于这里熵函数的简单形式，我们可以验证$s_{UU}$可以对称化$U$的方程。首先将$U$的方程写成下面的形式。 
\begin{eqnarray*}
U_t + \sum_{j=1}^n A_j(U) U_{x_j} = \mathcal{Q}(U),\\
  U = \left( \begin{array}{c} \rho \\ v_i \\ F_{kl} \\ c_{rs} \end{array} \right), \\
  A_j(U)= 
  \left( \begin{array}{cccc} v_j & \rho \delta_{i'j} & 0 &０ \\
    \frac{p_\rho}{\rho} \delta_{ij} & v_j \delta_{ii'} & -F_{jl'} \delta_{ik'}　& 0 \\
    0 & - F_{jl}\delta_{ki'}  & v_j \delta_{kk'} \delta_{ll'} & 0  \\
    0 & 0 & 0 & v_j \delta_{rr'} \delta_{ss'} \end{array} \right), \\
  \mathcal{Q}(U) = 
