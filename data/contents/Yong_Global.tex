...
下面考虑\eqref{eq:compressibleRelax}的Maxwell迭代一阶近似方程组的整体存在性。首先由\eqref{eq:compressibleRelax}中$c$的方程得到
\begin{eqnarray*}
...
下面我们将考虑该方程组在平衡态附近整体解的存在性。首先我们将说明该方程组可以看作对称双曲-抛物系统。从而其整体存在性可由Kawashima等人的理论\cite{kawashima1984systems,kawashima1985systems,yong2004entropy}进行研究。然而通过验证，Kawashima条件并不成立。幸运的是我们可以通过力学上的适应性条件来弥补这一缺陷，从而可以采用Kawashima的一般框架得到存在性定理的证明。
