...
\end{equation*}
此时假设$u = c_v T, s_0 = c_v \ln \theta$,那么可以得到$s = s (\theta,w)$的Hessian矩阵为
\begin{equation*}
	\left( \begin{array}{cc} 
...
\subsection{推广的Cattaneo定律}
 然而注意到$\alpha_0$假设是常数，所以这里我们需要$\frac{\xi}{\lambda \theta^2}$为常数。
如果上面的熵函数中$\alpha_0$依赖于内能（温度）$\alpha_0=\alpha_0(u)$，那么$q$的方程应写为
\begin{equation*}
({\alpha_0(u)}{q})_t-\nabla \theta^{-1}= M'{q}
\end{equation*}
