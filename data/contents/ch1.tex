...
\begin{theorem}
	...
	\end{eqnarray*}
\end{theorem}
在$L^p$框架下方程\eqref{eq:Oldroyda}的证明可以参见\cite{fernandez1998some}。Besov空间的存在性结果见\cite{chemin2001lifespan}。
...
另外一个重要的数学问题为$\lambda$趋于$0$时方程\eqref{eq:Oldroydb}的极限分析。形式上有$\lambda$很小，我们将$\sigma$的方程写成下面的形式
\begin{equation*}
...
\begin{theorem}
	令$n=2,3$，$(v_0,\sigma_0) \in H^s(\mathbf{R}^n) \times H^s(\mathbf{R}^{n^2})$
\end{theorem}
